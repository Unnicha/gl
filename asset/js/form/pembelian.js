	// var panggil = require('validasi')
	// import { panggil, validInput, validProduk } from "./validasi.js";
	...
	function showProduk() {
		...
		if (kode != '' && kode != undefined) {
			$.ajax({
				type	: 'post',
				url		: base_url+'pembelian/showProduk',
				data	: {
					kode_transaksi	: kode,
				},
	...
	$('.add-btn').click(function() {
		...
		$.ajax({
			type	: 'post',
			url		: base_url+'pembelian/addProduk',
			data	: {
	...
	$('tbody').on('click', '.btn-edit', function() {
		...
		$.ajax({
			type	: 'post',
			url		: base_url+'pembelian/editProduk',
			data	: {
				mata_uang	: $('#mata_uang').val(),
				id_row		: id,
	...
	$('tbody').on('click', '.btn-delete', function() {
		$.ajax({
			type	: 'post',
			url		: base_url+'pembelian/deleteProduk',
	...
	function getKonversi() {
		if ($('#tanggal').val() != '' && $('#mata_uang').val() != '') {
			$.ajax({
				type	: 'post',
				url		: base_url+'pembelian/getKonversi',
				data	: {
					tanggal : $('#tanggal').val(),
					mata_uang : $('#mata_uang').val(),
				},
	...
	// fungsi untuk menampilkan nilai konversi mata uang
	function getLawan() {
		if ($('#jenis_pembayaran').val() != '' && $('#supplier').val() != '') {
			$.ajax({
				type	: 'post',
				url		: base_url+'pembelian/getLawan',
				data	: {
	...
	function totalTransaksi() {
		...
		if (total_prod != undefined) {
			$.ajax({
				type : 'post',
				url : base_url+'pembelian/getTotal',
				data : {
	...
	function validInput() {
		...
		$('#myForm .form-control').each(function() {
			...
			if (field.attr('required') == 'required' && value.length == 0) {
				...
				// check ketersediaan faktur beli
				if (field.attr('unique') != undefined) {
					$.ajax({
						type : 'post',
						url : base_url+'pembelian/checkInvoice',
						data : {
							'invoice' : $('#faktur_beli').val(),
						},
