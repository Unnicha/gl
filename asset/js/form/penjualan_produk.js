...
function getDetailProduk() {
	var produk	= $('#kode').val();
	var qty		= $('#qty').val();
	var kode = $("input[name='kode_produk[]']").map(function() {
		if ($(this).val() == produk) return $(this).parents('tr').data('id');
	}).get()
	var selected = $("#qty_produk"+kode).val()
	
	$.ajax({
		type	: 'post',
		url		: base_url + 'penjualan/detailProduk',
		data	: {
			'kode'		: produk,
			'qty'		: qty,
			'selected'	: (selected == undefined) ? selected : (selected * -1),
		},
...
function submitProduk() {
	...
	if (kode.trim() == '') {
		...
		$.ajax({
			type	: 'POST',
			url		: (action == 'add') ? base_url+'penjualan/saveProduk' : base_url+'penjualan/updateProduk',
			data	: {
				'id_row'	: id,
				'nama'		: $('#nama').val(),
				'kode'		: kode,
