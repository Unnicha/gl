	...
	$.ajax({
		...
		success	: function(result) {
			if (result) {
				...
				$("#tgl_beli").val(data.tanggal_transaksi);
...
function submitInvoice() {
	...
	if (invoice == '') {
		...
		$.ajax({
			type	: 'POST',
			url		: base_url+'utang/saveUtang',
			data	: {
				'id_row'		: id_row,
				'invoice'		: invoice,
				'tanggal_beli'	: $('#tgl_beli').val(),
			...
			success	: function(result) {
				...
				if (action == 'saveUtang') {
					$('#listUtang').append(result);
				} else {
					$('#utangRow'+id_row).html(result);
				}
			},
			complete : function() {
				// totalUtang()
