...
<div class="modal-body">
	<?php $filter = $this->session->userdata('laporan_penjualan') ?>
			...
			<div class="form-group row">
				<div class="col">
					...
					<select name="jenis_transaksi" id="jenis_transaksi" data-err="#err-jenis_transaksi" class="form-control selectpicker">
						...
						<option value="penjualan" <?= $filter['jenis_transaksi'] == 'penjualan' ? 'selected' : ''; ?>>
							Penjualan
						</option>
		...
		<div class="col p-2">
			...
			<div class="form-group row">
				<div class="col">
					...
					<select name="pelanggan" id="pelanggan" data-err="#err-pelanggan" class="form-control selectpicker" data-live-search="true" data-size="7">
						<option value="">Semua Pelanggan</option>
						<?php foreach($pelanggan as $p) : ?>
						<option value="<?= $p['kode_pelanggan'] ?>" <?= $filter['pelanggan'] == $p['kode_pelanggan'] ? 'selected' : '' ?> >
							<?= $p['nama_pelanggan'] ?>
						</option>
						<?php endforeach ?>
					</select>
					<small class="text-danger" id="err-pelanggan"></small>
				</div>
			</div>
...
<script>
	...
	function tampilkan() {
		...
		if (tampil) {
			...
			$.ajax({
				type	: 'POST',
				url		: '<?= base_url() ?>laporan_penjualan/display',
				data	: {
					'id' : id,
					'value' : value,
				},
