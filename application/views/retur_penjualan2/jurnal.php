<div class="content container-fluid">
	...
	<div class="row">
		...
		<div class="col-auto">
			<a href="<?= base_url(); ?>retur_penjualan/tambah" class="btn btn-primary">
	...
	<div class="card shadow">
		<div class="card-header">
			<ul class="nav nav-tabs card-header-tabs" id="myTabs">
				<a class="nav-link" href="<?=base_url()?>retur_penjualan">Penjualan</a>
...
<script> 
	$(document).ready(function () {
		...
		var table = $('#myTable').DataTable({
			...
			'pageLength'	: 6,
			'ajax'			: {
				'url'	: '<?=base_url()?>retur_penjualan/table',
				'type'	: 'post',
				'data'	: function (e) { 
					e.tab = 'jurnal';
				},
			...
			'columns' : [
				...
				{ name: 'retur_penjualan.kode_transaksi' },
				{ name: 'kode_akun' },
				{ name: 'nama_akun' },
				{ name: 'debit' },
				{ name: 'kredit' },
		...
		$('#myTable').on('click', '.btn-detail', function() {
			$.ajax({
				type	: 'POST',
				url		: '<?= base_url(); ?>retur_penjualan/detail',
				data	: {
					'id' : $(this).data('id'),
				},
		...
		$('#myTable').on('click', '.btn-hapus', function() {
			$.ajax({
				type	: 'POST',
				url		: '<?= base_url(); ?>retur_penjualan/delete',
				data	: {
					'id' : $(this).data('id'),
				},
				success	: function(data) {
