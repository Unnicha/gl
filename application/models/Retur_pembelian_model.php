<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Retur_pembelian_model extends CI_Model 
	{
		...
		public function getTransaksi($cari='', $offset=0, $limit='', $order='', $bulan='') 
		{
			...
			$total	= 'SELECT SUM((harga_produk - diskon_produk) * qty_produk) FROM retur_pembelian_produk WHERE retur_pembelian_produk.kode_transaksi = retur_pembelian.kode_transaksi';
			return $this->db->select('retur_pembelian.*, supplier.*, kode_asal.nama_akun AS nama_asal, kode_lawan.nama_akun AS nama_lawan, ('.$total.') AS total_produk')
							->join('supplier', 'supplier.kode_supplier = retur_pembelian.supplier', 'left')
							->join('akun_perkiraan AS kode_asal', 'retur_pembelian.akun_asal = kode_asal.kode_akun', 'left')
							->join('akun_perkiraan AS kode_lawan', 'retur_pembelian.akun_lawan = kode_lawan.kode_akun', 'left')
							->get('retur_pembelian')->result_array();
		}
		...
		public function countTransaksi($cari='', $bulan='') 
		{
			...
			return $this->db->select('retur_pembelian.*')
							->join('akun_perkiraan AS kode_asal', 'retur_pembelian.akun_asal = kode_asal.kode_akun')
							->join('akun_perkiraan AS kode_lawan', 'retur_pembelian.akun_lawan = kode_lawan.kode_akun')
							->from('retur_pembelian')->count_all_results();
		}
		
		public function getJurnal($cari='', $offset=0, $limit='', $order='', $bulan='') 
		{
			if ($cari) {
				$this->db->like('retur_pembelian.kode_transaksi', $cari)->or_like('tanggal_transaksi', $cari)
			...
			$total	= 'SELECT SUM((harga_produk - diskon_produk) * qty_produk) FROM retur_pembelian_produk WHERE retur_pembelian_produk.kode_transaksi = retur_pembelian.kode_transaksi';
			$produk	= 'SELECT kode_produk FROM retur_pembelian_produk WHERE retur_pembelian_produk.kode_transaksi = retur_pembelian.kode_transaksi';
			$urut	= 'CASE WHEN kode_akun = akun_asal THEN 1 WHEN kode_akun = akun_ppn THEN 3 WHEN kode_akun = akun_lawan THEN 4 ELSE 2 END';
			return $this->db->distinct()
							->select('kode_akun, nama_akun, retur_pembelian.*, ('.$total.') AS total, ('.$urut.') AS urut')
		...
		{
			if ($cari) {
				$this->db->like('tanggal_transaksi', $cari)->or_like('retur_pembelian.kode_transaksi', $cari)
						->or_like('kode_akun', $cari)->or_like('nama_akun', $cari);
			}
			if ($bulan) $this->db->like('tanggal_transaksi', $bulan, 'after');
			...
			$produk	= 'SELECT kode_produk FROM retur_pembelian_produk WHERE retur_pembelian_produk.kode_transaksi = retur_pembelian.kode_transaksi';
			return $this->db->distinct()
							->select('kode_akun, retur_pembelian.*')
							->join('akun_perkiraan', 'kode_akun = akun_asal OR kode_akun IN ('.$produk.') OR kode_akun = akun_ppn OR kode_akun = akun_lawan', 'left')
							->from('retur_pembelian')->count_all_results();
		}
		...
		public function getById($id) 
		{
			return $this->db->select('*, kode_asal.nama_akun AS nama_asal, kode_lawan.nama_akun AS nama_lawan, kode_ppn.nama_akun AS nama_ppn')
							->join('akun_perkiraan AS kode_asal', 'retur_pembelian.akun_asal = kode_asal.kode_akun', 'left')
							->join('akun_perkiraan AS kode_lawan', 'retur_pembelian.akun_lawan = kode_lawan.kode_akun', 'left')
							->join('akun_perkiraan AS kode_ppn', 'retur_pembelian.akun_ppn = kode_ppn.kode_akun', 'left')
							->join('supplier', 'retur_pembelian.supplier = supplier.kode_supplier', 'left')
							->where('kode_transaksi', $id)
							->get('retur_pembelian')->row_array();
		}
		
		public function getProduk($id_transaksi) 
		{
			return $this->db->where('kode_transaksi', $id_transaksi)
							->get('retur_pembelian_produk')->result_array();
		}
		
		public function getByInvoice($faktur_retur_beli)
		{
			return $this->db->where('faktur_retur_beli', $faktur_retur_beli)
							->get('retur_pembelian')->row_array();
		}
		...
		public function add() 
		{
			...
			$retur = [
				...
				'faktur_beli'		=> $faktur_beli,
				'faktur_retur_beli'	=> $this->input->post('faktur_retur', true),
			...
			// // insert transaksi
			$this->db->insert('retur_pembelian', $retur);
			
			// insert produk
			$produk = $this->setProduk($kode_transaksi, $faktur_beli);
			$this->db->insert_batch('retur_pembelian_produk', $produk);
		...
		public function edit() 
		{
			...
			// insert transaksi
			$this->db->update('retur_pembelian', $transaksi, ['kode_transaksi' => $kode_transaksi]);
			
			// hapus produk yang sudah disimpan
			$this->db->delete('retur_pembelian_produk', ['kode_transaksi' => $kode_transaksi]);
		...
		public function setProduk($kode_transaksi, $faktur_beli) 
		{
			$produk	= [];
		...
		{
			$table	= ['retur_pembelian', 'retur_pembelian_produk'];
