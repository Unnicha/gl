<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class M_laporan_hutang extends CI_Model 
	{
		...
		public function getHutang($cari='', $offset=0, $limit='', $order='', $filter)
		{
			if($cari) {
			...
			$bayar	= 'SELECT SUM(jumlah_bayar) FROM hutang_bayar WHERE hutang_bayar.faktur_jual = penjualan.faktur_jual';
			...
			if ($filter['jenis_laporan'] == 'hutang') {
				...
				return $this->db->select('penjualan.*, hutang.*, jumlah_bayar,  
								...
								->join('hutang_bayar', 'hutang_bayar.faktur_jual = penjualan.faktur_jual', 'left')
								->join('hutang', 'hutang.kode_bayar = hutang_bayar.kode_bayar', 'left')
								->join('akun_perkiraan AS akun_lawan', 'akun_lawan.kode_akun = hutang.akun_asal', 'left')
								->where([
									'jenis_pembayaran' => 'Kredit', 
									'hutang.kode_bayar != ' => NULL,
									'tanggal_transaksi >= ' => $filter['tanggal_awal'], 
									'tanggal_transaksi <= ' => $filter['tanggal_akhir']
									])
		...
		public function countHutang($cari='', $filter)
		{
			if($cari) {
				$this->db->like('kode_bayar', $cari)->or_like('tanggal_bayar', $cari);
			}
			...
			if ($filter['jenis_laporan'] == 'hutang') {
				...
				return $this->db->select('penjualan.*, hutang.*')
								->join('akun_perkiraan AS akun_asal', 'akun_asal.kode_akun = penjualan.akun_asal', 'left')
								->join('hutang_bayar', 'hutang_bayar.faktur_jual = penjualan.faktur_jual', 'left')
								->join('hutang', 'hutang.kode_bayar = hutang_bayar.kode_bayar', 'left')
								->join('akun_perkiraan AS akun_bayar', 'akun_bayar.kode_akun = hutang.akun_asal', 'left')
								->where([
									'jenis_pembayaran' => 'Kredit', 
									'hutang.kode_bayar != ' => NULL,
									'tanggal_transaksi >= ' => $filter['tanggal_awal'], 
