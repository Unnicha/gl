<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class M_laporan_penjualan extends CI_Model 
	{
		...
		public function getDetail($cari='', $offset=0, $limit='', $order='', $filter)
		{
			...
			$table1	= $filter['jenis_transaksi'] == 'penjualan' ? 'penjualan' : 'retur_penjualan';
			...
			$diskon	= ($filter['jenis_transaksi'] == 'penjualan') ? 'diskon_luar' : '0';
			...
			return $this->db->select('*, 
								...
								')
							->join('pelanggan', 'pelanggan.kode_pelanggan = '.$table1.'.pelanggan', 'left')
		...
		public function countDetail($cari='', $filter)
		{
			...
			return $this->db->join('pelanggan', 'pelanggan.kode_pelanggan = '.$table1.'.pelanggan', 'left')
							->join($table2, $table2.'.kode_transaksi = '.$table1.'.kode_transaksi', 'left')
							->join('barang', $table2.'.kode_produk = barang.kode_barang', 'left')
							->where([
								'tanggal_transaksi >= ' => $filter['tanggal_awal'], 
