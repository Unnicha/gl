<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Pembelian extends CI_Controller 
	{
		...
		public function index()
		{
			...
			$this->libtemplate->main('pembelian/index', $data);
		}
		
		public function jurnal()
		{
			$data['title'] = 'Pembelian';
			$this->libtemplate->main('pembelian/jurnal', $data);
		}
		...
		public function table()
		{
			...
			// table transaksi
			if($tab == 'transaksi') {
				$transaksi	= $this->pembelian->getTransaksi($cari, $offset, $limit, $order, $bulan);
				$countData	= $this->pembelian->countTransaksi($cari, $bulan);
				
				foreach($transaksi as $i) {
					$invoice	= $this->pembelian->getByInvoice($i['faktur_beli']);
					...
					$btn = '
						<a href="'.base_url('pembelian/edit/'.$i['kode_transaksi']).'" class="badge badge-info badge-action" data-toggle="tooltip" data-placement="left" title="Ubah">
					...
					$btn_2	= '
						<a href="'.base_url('retur_pembelian/tambah/'.$i['kode_transaksi']).'" class="badge badge-warning badge-action" data-toggle="tooltip" data-placement="left" title="Retur">
							<i class="bi bi-arrow-left-right icon-medium"></i>
						</a>';
					$btn_3	= '
						<a href="'.base_url('utang/tambah/'.$i['kode_transaksi']).'" class="badge badge-success badge-action" data-toggle="tooltip" data-placement="left" title="Pembayaran">
			...
			// table jurnal
				...
				$transaksi	= $this->pembelian->getJurnal($cari, $offset, $limit, $order, $bulan);
				$countData	= $this->pembelian->countJurnal($cari, $bulan);
		...
		{
			$akun_ppn	= $this->akun->getByJenis('44');
			if ($akun_ppn) {
				$data['title']			= 'Tambah Pembelian';
				$data['supplier']		= $this->supplier->getAll();
				$data['akun_asal']		= $this->akun->getByJenis(['41']);
				...
				$this->form_validation->set_rules('faktur_beli', 'Faktur Jual', 'required');
				...
				if($this->form_validation->run() == FALSE) {
					$this->libtemplate->main('pembelian/tambah', $data);
				} else {
					if ($this->pembelian->add() > 0) {
						$this->session->set_flashdata('notification', 'Berhasil ditambahkan!');
					}
					redirect($this->redirect);
				}
			} else {
				echo 'Akun Pajak Masukan belum ada.';
			}
		...
		{
			$akun_ppn	= $this->akun->getByJenis('44');
			$transaksi	= $this->pembelian->getById($id);
			...
			if($this->form_validation->run() == FALSE) {
				$this->libtemplate->main('pembelian/edit', $data);
			} else {
				if ($this->pembelian->edit() > 0) {
		...
		public function detail()
		{
			...
			$produk			= $this->pembelian->getProduk($kode_transaksi);
			$transaksi		= $this->pembelian->getById($kode_transaksi);
		...
		public function delete()
		{
			...
			$data['button']	= '
				<a href="pembelian/fixDelete/'.$id.'" class="btn btn-danger">Hapus</a>
		...
		public function checkInvoice()
		{
			...
			$invoice = $this->pembelian->getByInvoice($invoice);
			if ($invoice) {
				$result = 'false';
			}
			echo $result;
		...
		public function getLawan()
		{
			...
			if ($jenis_trx == 'Kredit')		$akun = $supplier['akun_utang'];
			else if ($jenis_trx == 'Tunai')	$akun = $supplier['akun_kas'];
			else							$akun = $supplier['akun_bank'];
		...
		{
			$row	= '';
			$produk	= $this->pembelian->getProduk($_REQUEST['kode_transaksi']);
			foreach ($produk as $key => $p) {
		...
		public function addProduk()
		{
			...
			$this->load->view('pembelian/produk_tambah', $data);
		}
		...
		public function editProduk()
		{
			...
			$this->load->view('pembelian/produk_edit', $data);
		}
		
		/**
		 * simpan produk
		 * temporary, hanya untuk form pembelian, bukan database
		 */
		public function saveProduk()
		{
			$produk = $this->input->post();
			$produk['nama']	= $this->barang->getById($produk['kode'])['nama_barang'];
