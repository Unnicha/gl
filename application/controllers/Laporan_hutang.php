<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Laporan_hutang extends CI_Controller 
	{
		public function __construct()
		{
			...
			$this->load->model('M_laporan_hutang', 'laporan');
		...
		public function index() {
			// $this->session->unset_userdata('laporan_hutang');
			$data['title']	= 'Laporan Hutang & Pembayaran';
			$this->libtemplate->main('laporan_hutang/index', $data);
		}

		public function ganti()
		{
			$data['akun_asal']	= $this->akun->getByJenis('43');
			$data['mata_uang']	= $this->mata_uang->getAll();
			$data['min_date']	= $this->min_date;
			$data['max_date']	= $this->max_date;
			
			$this->load->view('laporan_hutang/filter', $data);
		}
		...
		public function display()
		{
			...
			$this->session->set_userdata('laporan_hutang', $filter);
			
			$transaksi	= ($filter['jenis_laporan'] == 'hutang') ? 'Hutang' : 'Pembayaran Hutang';
			...
			$this->load->view('laporan_hutang/'.$filter['jenis_laporan'], $data);
		}
		...
		public function table()
		{
			...
			$filter	= $this->session->userdata('laporan_hutang');
			$filter['tanggal_awal']		= Globals::dateFormat($filter['tanggal_awal']);
			$filter['tanggal_akhir']	= Globals::dateFormat($filter['tanggal_akhir']);
			
			$transaksi	= $this->laporan->getHutang($cari, $offset, $limit, $order, $filter);
			$countData	= $this->laporan->countHutang($cari, $filter);
			
			$data = [];
			// table laporan hutang
			if ($filter['jenis_laporan'] == 'hutang') {
			...
			// end table laporan bayar hutang
